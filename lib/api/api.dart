...
class Api {
  static String upComingUrl = 'https://api.themoviedb.org/3/movie/upcoming?api_key=${ApiConstant
      .apiKey}';
  static String popularUrl = 'https://api.themoviedb.org/3/movie/popular?api_key=${ApiConstant
      .apiKey}';
  static String topRatedUrl = 'https://api.themoviedb.org/3/movie/top_rated?api_key=${ApiConstant
      .apiKey}';
  static String SearchUrl = 'https://api.themoviedb.org/3/search/movie?api_key=${ApiConstant
      .apiKey}';
  ...
  static Future <List<Results>> getUpComingMovies() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Results> movies = data.map((movie) => Results.fromJson(movie))
          .toList();
      return movies;
    } else {
  ...
  static Future<DetailsResponse> getDetailsAboutMovie(int movieId) async {
    final response = await http.get(
        Uri.parse(
            'https://api.themoviedb.org/3/movie/$movieId?api_key=${ApiConstant
                .apiKey}')
    );
    if (response.statusCode == 200) {
  ...
  static Future <List<Results>> getMoreLikeThis(int movieId) async {
    final response = await http.get(
        Uri.parse(
            'https://api.themoviedb.org/3/movie/$movieId/similar?api_key=${ApiConstant
                .apiKey}'));
    if (response.statusCode == 200) {
      final List<dynamic> data = jsonDecode(response.body)['results'];
      List<Results> movies = data.map((movie) => Results.fromJson(movie))
  ...
  static Future<List<SearchResults>> searchList(String searchVal) async {
    final response = await http.get(
        Uri.parse('${SearchUrl}&query=${searchVal}')
    );
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
      var searchJson = data['results'] as List;
      List<SearchResults> searchResultsList = searchJson.map((json) =>
          SearchResults.fromJson(json)).toList();
